            ...
            @if($product->photos->count())
                ...
                <div class="row" style="margin-top: 20px;">
                    @foreach($product->photos as $photo)
                        <div class="col-4">
                            <img src="{{ asset('storage/' . $photo->image) }}" alt="" class="img-fluid img-small border-dark border-2">
                        </div>
                    @endforeach
                </div>
            @else
    ...
    <script>
        ...
        imgSmall.forEach(function (el) {
            el.addEventListener('click', function (e) {
                let activeImg = document.querySelector('.img-small.active');

                if (activeImg){
                    activeImg.classList.remove('border');
